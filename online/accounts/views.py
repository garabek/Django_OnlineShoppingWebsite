...
def logout_view(request):
	logout(request)
	messages.success(request, "Successfully Logged Out. Feel free to <a href='%s'>Login</a> again." 
		%(reverse("auth_login")), extra_tags='safe')
	#messages.warning(request, "There's a warning!!!")
	#messages.error(request, "There's a warning!!!")
...
def registration_view(request):
	...
	if form.is_valid():
		...
		#new_user.first_name = "Kaygisiz"
		new_user.save()
		messages.success(request, "Successfully Registered. Please Confirm you email.")
		return HttpResponseRedirect("/")
		# username = form.cleaned_data['username']
		# password = form.cleaned_data['password']
		# user = authenticate(username=username, password=password)
...
def activation_view(request, activation_key):
	if SHA1_RE.search(activation_key):
		...
		try:
			...
			instance = None
			messages.success(request, "There was an error with your rewuest.")
			return HttpResponseRedirect("/")
		if instance is not None and not instance.confirmed:
			page_message = "Confirmation Succesful! Welcome." 
			instance.confirmed = True
			instance.activation_key = "Confirmed"
			instance.save()
			messages.success(request, "Successfully Confirmed. Please Log In")
